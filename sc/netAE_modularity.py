...
def make_aff_matrix(features, gamma):
    ...
    aff_matrix : tensor, shape (n_samples, n_samples)
        aff_matrix[i, j] is the affinity score of sample i and j measured by the features that represent these two samples.
...
def calc_modularity_mult(partitions, gamma):
    ...
    for features, labels in partitions:
        ...
        # 当遇到只有一行时会计算出模块的值为nan, 将其置为0
        if(delta_q != delta_q):
            delta_q = torch.tensor(0).double()
