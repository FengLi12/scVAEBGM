...
def elbo_SCALE(recon_x, x, gamma, c_params, z_params, binary=True):
    ...
    if binary:
        ...
        likelihood = -F.mse_loss(recon_x, x, size_average=False)
        # print("\nmse_loss:", likelihood)
    ...
    if binary:
        ...
        return likelihood, torch.sum(kld)
    # return likelihood, torch.sum(kld)  # 使用mse_loss损失时(False)
    # return torch.sum(likelihood), torch.sum(kld)   #使用binary_cross_entropy损失时(True)
